      ...
      <v-container>
        <v-layout row wrap>
          <v-radio-group v-model="radios" :mandatory="false" row>
            <v-radio id="input-radio-cpf" label="CPF" value="cpf"></v-radio>
            <v-radio id="input-radio-tel" label="Telefone" value="telefone"></v-radio>
          </v-radio-group>
          <v-flex xs12>
            <v-text-field
...
export default {
  ...
  methods: {
    ...
    consultar() {
      this.$store.dispatch("cliente/buscarCliente", {
        dado: this.dado,
        tipo: this.radios
      });
